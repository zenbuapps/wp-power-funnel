@ignore @command
Feature: 觸發 WORKFLOW_FAILED 觸發點
  ...
  Rule: 前置（狀態）- Workflow 狀態必須從 running 轉為 failed

    Example: 工作流節點執行拋出例外後觸發
      Given 工作流 301 的狀態為 "running"
      When 系統將工作流 301 的狀態更新為 "failed"
      Then 系統應觸發 "pf/trigger/workflow_failed"
  ...
  Rule: 前置（狀態）- Workflow 狀態未變更為 failed 時不應觸發

    Example: 工作流狀態從 running 轉為 completed 時不觸發 WORKFLOW_FAILED
      Given 工作流 301 的狀態為 "running"
      When 系統將工作流 301 的狀態更新為 "completed"
      Then 系統不應觸發 "pf/trigger/workflow_failed"
